...
class WebViewBridge {
  ...
  private messageHandlers: Map<EditorMessage['type'], (payload: any) => void> =
    new Map();
  ...
  private setupListeners() {
    const handleMessage = (e: MessageEvent) => {
      try {
        const isDataString = typeof e.data === 'string';
        const parsedData = isDataString ? JSON.parse(e.data) : e.data;
        const message = parsedData as EditorMessage;
