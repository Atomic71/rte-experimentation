...
export const DirectionPlugin: React.FC = () => {
  const [editor] = useLexicalComposerContext()
  const [currentDirection, setCurrentDirection] = useState<'ltr' | 'rtl' | 'auto'>('ltr')
  ...
  const setDirection = useCallback((direction: 'ltr' | 'rtl' | 'auto') => {
    editor.update(() => {
  ...
  return (
    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
      <button
        style={currentDirection === 'ltr' && !isAutoMode ? activeButtonStyle : buttonStyle}
      ...
      <button
        style={currentDirection === 'rtl' && !isAutoMode ? activeButtonStyle : buttonStyle}
...
export const useDirection = () => {
  ...
  const setDirectionManually = useCallback((direction: 'ltr' | 'rtl' | 'auto') => {
    editor.update(() => {
      $setDirection(direction)
    })
  }, [editor])
